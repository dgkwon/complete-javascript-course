...
//     })
//     .finally(() => {
//       // can be used to remove spinner for example...
//       countriesContainer.style.opacity = 1;
//     });
// };
    ...
    .then(response => {
      if (!response.ok) {
        if (response.status === 403)
          throw new Error(
            'Slow down. Three requests are only accepted in 1 second.'
          );
...
const whereAmI = async function () {
  try {
    ...
    // Reject pormise returned from async function
    throw err;
  }
};
...
const loadAll = function (imgArr) {
  ...
  Promise.all(imgs).then(images => {
    images.map(img => img.classList.add('paralell'));
  });
};
